 ...
 * Teste tous les syst�mes: Director, Agents, Optimizer, Training
 */
...
async function runGlobalTest() {
    ...
    try {
        ...
        if (data.success && data.agents.length >= 7) {
            console.log('    PASS - 7 agents configur�s');
    ...
    try {
        ...
        const data = await res.json();
        if (data.success) {
            console.log(`    PASS - ${data.count} commentaires trouv�s`);
    ...
    try {
        ...
        if (data.success && data.response) {
            ...
            console.log(`    R�ponse: ${data.response.substring(0, 100)}...`);
    ...
    try {
        ...
        if (modelCount > 0) {
            console.log(`    PASS - ${modelCount} mod�les track�s`);
    ...
    // Summary
    console.log('\n' + '='.repeat(50));
    console.log(` R�SULTATS: ${results.passed} PASS / ${results.failed} FAIL`);
