...
const Avatar = ({ isSpeaking, size = "w-12 h-12", className = "" }) => {
  return (
    <div className={`relative ${size} ${className} group ${isSpeaking ? 'animate-pulse' : ''}`}>
      ...
      <div className={`relative w-full h-full rounded-full overflow-hidden border-2 ${isSpeaking ? 'border-cyan-400 shadow-[0_0_30px_rgba(34,211,238,0.8)]' : 'border-cyan-900 shadow-[0_0_10px_rgba(8,145,178,0.3)]'} transition-all duration-300`}>
        <img
          src="/agent33.jpg"
          alt="Thirty3"
          className={`w-full h-full object-cover object-top filter contrast-125 brightness-90 grayscale hover:grayscale-0 transition-all duration-500 ${isSpeaking ? 'scale-110 brightness-110 grayscale-0' : ''}`}
        />
