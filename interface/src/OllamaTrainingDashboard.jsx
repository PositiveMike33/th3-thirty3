...
const isAllowedModel = (modelName) => {
    ...
    // 4. Cloud Models (Provider in brackets)
    if (modelName.trim().startsWith('[')) return true;

    // 5. Allow Gemini/OpenAI explicit names
    if (lowerName.includes('gemini') || lowerName.includes('gpt')) return true;
...
const OllamaTrainingDashboard = () => {
    ...
    const [allRawMetrics, setAllRawMetrics] = useState({}); // Debug: Store all raw metrics
    const [showDebug, setShowDebug] = useState(false); // Enable debug mode
    ...
    const fetchMetrics = useCallback(async () => {
        try {
            ...
            setAllRawMetrics(data); // Debug output
    ...
    return (
        <div className="flex-1 p-6 bg-black text-cyan-300 overflow-y-auto overflow-x-hidden bg-[url('/grid.png')]">
            ...
            {/* Debug Panel */}
            {showDebug && (
                <div className="mb-6 p-4 bg-red-900/20 border border-red-800 rounded-xl overflow-x-auto">
                    <h3 className="text-red-400 font-bold mb-2">MODE DEBUG - Tous les modèles bruts reçus de l'API</h3>
                    <div className="text-xs font-mono text-gray-300 whitespace-pre">
                        {JSON.stringify(Object.keys(allRawMetrics), null, 2)}
                    </div>
                </div>
            )}
