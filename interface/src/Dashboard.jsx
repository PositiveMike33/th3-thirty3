...
const Dashboard = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                // Fetch Portfolio via centralized API service
                const portData = await api.get('/finance/portfolio');
                ...
                for (const sym of SYMBOLS) {
                    try {
                        const tickData = await api.get(`/finance/ticker?symbol=${sym}`);
                ...
                // Fetch News
                const newsData = await api.get('/finance/news');
        ...
        <div className="h-full overflow-y-auto p-6 bg-gray-900 text-cyan-300 font-mono">
            ...
            <div className="flex gap-4 mb-6">
                <button
                    ...
                    className={`px-6 py-3 rounded-lg font-bold transition-all ${activeTab === 'portfolio'
                ...
                <button
                    ...
                    className={`px-6 py-3 rounded-lg font-bold transition-all ${activeTab === 'payments'
                            ? 'bg-cyan-600 text-white'
                            : 'bg-gray-800 text-gray-400 hover:bg-gray-700'
                        }`}
                >
